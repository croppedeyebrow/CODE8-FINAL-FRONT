...
const PerformanceDetail = () => {
  ...
  useEffect(() => {
    setFooterData([
      <a href="https://www.flaticon.com/kr/free-icons/" title="연필 아이콘">
        연필 아이콘 제작자: Pixel perfect - Flaticon{" "}
      </a>,
      <br />,
      <a href="https://www.flaticon.com/kr/free-icons/" title="연필 아이콘">
        연필 zz아이콘 제작자: Pixel perfect - Flaticon
      </a>,
    ]);
  }, []);
  ...
  // 현재 시간과 공연 시간을 비교합니다.
  const isEnded =
    new Date() > new Date(performance && performance.performanceDate);
  ...
  useEffect(() => {
    ...
    const getPerformanceList = async () => {
      try {
        ...
        const matchingData = res.data.filter(
          (item) => item.performanceId === Number(id)
        ); // item은 res.data의 요소 하나하나를 의미
  ...
  useEffect(() => {
    const getMemberInfo = async () => {
      try {
        ...
        const matchingUser = userRes.data.filter((user) =>
          performance.nicknames.some(
            (nickname) => nickname === user.userNickname
          )
        );
  ...
  const email = UseAuth();
  const checkLogin = () => {
    // 로컬스토리지상에서 이메일, accessToken, refreshToken이 있는지 확인
    if (!email) {
  ...
  const deletePerformance = async () => {
    try {
      const res = await PerformanceAxios.deletePerformance(
  ...
  return (
    <>
      <Container>
        ...
        <Information>
          <div className="title">
            {performance && performance.performanceName}
          </div>
          <div className="info">
            <div className="venue">
              공연 장소 :{" "}
              {performance &&
                performance.venue + ", " + performance.detailVenue}
            </div>
            <div className="date">
              일시 : {performance && performance.performanceDate}
            </div>
            <div className="seat">
              좌석 수 : {performance && performance.seatCount}
            </div>
            <div className="desc">{performance && performance.description}</div>
            <div className="delete">
              {memberInfo &&
                memberInfo.some((user) => user.userEmail === email) && (
          ...
          <div className="map">
            {performance && (
              <KakaomapComponent performanceList={[performance]} />
            )}
          </div>
        </Information>
        <Bottom>
